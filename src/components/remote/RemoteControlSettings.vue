...
async function saveConfig() {
  ...
  if (!url) {
    toast.error(t('remote.settings.toast.relayRequired'))
    return
  }
  ...
  try {
    ...
    toast.success(t('remote.settings.toast.settingsSaved'))
  } catch (e) {
    toast.error(String(e))
  } finally {
...
async function toggleEnabled() {
  ...
  try {
    if (enabled.value) {
      await store.disable()
      toast.success(t('remote.settings.toast.disabled'))
    } else {
      await store.enable()
      toast.success(t('remote.settings.toast.enabled'))
    }
  } catch (e) {
    toast.error(String(e))
  } finally {
...
async function clearMasterPassword() {
  ...
  const ok = await confirm({
    title: t('remote.settings.confirmClearMaster.title'),
    message: t('remote.settings.confirmClearMaster.message'),
    confirmLabel: t('remote.settings.confirmClearMaster.confirmLabel'),
    variant: 'destructive',
  })
  if (!ok) return
  ...
  try {
    ...
    masterPassword.value = ''
    toast.success(t('remote.settings.toast.masterCleared'))
  } catch (e) {
    toast.error(String(e))
  } finally {
...
async function clearAudit() {
  ...
  const ok = await confirm({
    title: t('remote.settings.confirmClearAudit.title'),
    message: t('remote.settings.confirmClearAudit.message'),
    confirmLabel: t('remote.settings.confirmClearAudit.confirmLabel'),
    variant: 'destructive',
  })
  if (!ok) return
  ...
  try {
    ...
    toast.success(
      removed > 0
        ? t('remote.settings.toast.auditCleared', { count: removed })
        : t('remote.settings.toast.auditAlreadyEmpty'),
    )
  } catch (e) {
    toast.error(String(e))
  } finally {
  ...
  <div class="space-y-6">
    ...
    <Card>
      <template #header>
        ...
        <Badge v-else tone="neutral">{{ t('remote.settings.off') }}</Badge>
      </template>
      <div class="p-4 space-y-4">
        ...
        >
          <div class="min-w-0">
            <p class="text-sm font-medium text-foreground">{{ t('remote.settings.enable') }}</p>
            <p class="mt-0.5 text-[11px] text-muted-foreground">
              <template v-if="enabled && connected">{{ t('remote.settings.state.connected') }}</template>
              <template v-else-if="enabled">{{ t('remote.settings.state.running') }}</template>
              <template v-else-if="!canToggle">{{ t('remote.settings.state.needsRelay') }}</template>
              <template v-else>{{ t('remote.settings.state.off') }}</template>
            </p>
          </div>
          <button
            type="button"
        ...
        <div class="space-y-1.5">
          <label class="text-xs font-medium text-muted-foreground">{{ t('remote.settings.relayUrl') }}</label>
          <Input
            v-model="relayUrl"
            :placeholder="t('remote.settings.relayUrlPlaceholder')"
        ...
        <div class="space-y-1.5">
          <label class="text-xs font-medium text-muted-foreground">
            {{ t('remote.settings.authToken') }}
            <span v-if="status?.has_auth_token" class="text-emerald-500">{{ t('remote.settings.saved') }}</span>
          </label>
          <Input
            v-model="authToken"
            type="password"
            :placeholder="t('remote.settings.authTokenPlaceholder')"
            :disabled="savingConfig"
          />
          <p class="text-[11px] text-muted-foreground/70">
            {{ t('remote.settings.authTokenHint') }}
          </p>
        </div>
        ...
        <div class="space-y-1.5">
          <label class="flex items-center gap-2 text-xs font-medium text-muted-foreground">
            <span>
              {{ t('remote.settings.masterPassword') }}
              <span v-if="status?.has_master_password" class="text-emerald-500">{{ t('remote.settings.set') }}</span>
            </span>
            <button
          ...
          <Input
            ...
            :placeholder="status?.has_master_password
              ? t('remote.settings.masterPasswordReplace')
              : t('remote.settings.masterPasswordSet')"
            :disabled="savingConfig || savingMasterPw"
            @keyup.enter="saveConfig"
          />
          <p class="text-[11px] text-muted-foreground/70">
            {{ t('remote.settings.masterPasswordHint') }}
          </p>
        </div>

        <div class="flex items-center gap-2 pt-1">
          <Button variant="outline" :disabled="savingConfig" @click="saveConfig">
            {{ t('remote.settings.saveSettings') }}
          </Button>
          <span v-if="status?.device_id" class="ml-auto text-[11px] text-muted-foreground/60 font-mono">
            {{ t('remote.settings.hostLabel', { id: status.device_id.slice(0, 8) }) }}
          </span>
        </div>

        <p class="text-[11px] text-muted-foreground/70 border-t border-border/50 pt-3">
          <i18n-t keypath="remote.settings.perRunHint" tag="span">
            <template #remote>
              <span class="font-medium text-foreground/80">{{ t('remote.settings.remote') }}</span>
            </template>
            <template #masterPassword>
              <span class="font-medium text-foreground/80">{{ t('remote.settings.masterPasswordInline') }}</span>
            </template>
          </i18n-t>
        </p>
      </div>
    ...
    <Card>
      <template #header>
        ...
        <h3 class="text-sm font-semibold flex-1">{{ t('remote.settings.auditLog') }}</h3>
        <div class="flex items-center gap-1">
          ...
          <button
            ...
            :title="t('remote.settings.clearLog')"
      ...
      <div v-if="store.audit.length" class="max-h-80 overflow-auto">
        <table class="w-full text-xs">
          <thead class="sticky top-0 bg-muted/40 text-muted-foreground">
            <tr>
              <th class="px-3 py-2 text-left font-medium">{{ t('remote.settings.table.time') }}</th>
              <th class="px-3 py-2 text-left font-medium">{{ t('remote.settings.table.action') }}</th>
              <th class="px-3 py-2 text-left font-medium">{{ t('remote.settings.table.result') }}</th>
              <th class="px-3 py-2 text-left font-medium">{{ t('remote.settings.table.run') }}</th>
            </tr>
          </thead>
